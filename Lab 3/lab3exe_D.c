...
int substring(const char *s1, const char* s2)
{
    ...
    if(s2[0] == '\0') //a check for the case that the string is just empty
        return 0;

    for(int i = 0; s1[i] != '\0'; i++) //loops until the entire main string is checked
    {
        if(s1[i] == s2[charNum]) //if a point in the string is equal to the substring, check the next substring's letter next loop
            charNum ++;
        else if(s2[charNum] == '\0') //if the substring reaches null that means it's passed!
            return 1; //yay!
        else if(s1[i] != s2[charNum]) //during the read if the substring differs from the main string, start over
            charNum = 0;
    }

    return 0; //if success wasn't found in the loop, returns 0
}
...
void select_negatives(const int *source, int n_source, int* negatives_only, int* number_of_negatives)
{
    ...
    *number_of_negatives = 0;
    ...
    for(i = 0; i < n_source; i++) //for loop that scans the entire source array
    {
        if(source[i] < 0) //if the current array value is negative
        {
            negatives_only[*number_of_negatives] = source[i]; //add that value to the negatives_only array
            *number_of_negatives = *number_of_negatives + 1; //and increase the number of negatives by one
        }
    }
